...
def writeReport(xmlfile, dataType, numDown, seqType):
    ...
    if dataType == "run":
        if numDown:
            for item in islice(root.iterfind("RUN/IDENTIFIERS/PRIMARY_ID"), 0,
                               int(numDown)):
        ...
        if numDown:
            for item in islice(root.iterfind("STUDY/IDENTIFIERS/PRIMARY_ID"),
                               0, int(numDown)):
                accessID.append(item.text)
        ...
        if numDown:
            for item in islice(
                    root.iterfind("EXPERIMENT/IDENTIFIERS/PRIMARY_ID"), 0,
                    int(numDown)):
                accessID.append(item.text)
            for item in islice(root.iter("TITLE"), 0, int(numDown)):
                title.append(item.text)
        ...
        if numDown:
            for item in islice(root.iterfind("SAMPLE/IDENTIFIERS/PRIMARY_ID"),
                               0, int(numDown)):
                accessID.append(item.text)
            for item in islice(root.iter("TITLE"), 0, int(numDown)):
                title.append(item.text)
    ...
    for item in accessID:
        enaURL.append(
            "https://www.ebi.ac.uk/ena/browser/view/{0}".format(item))
